# File: client.py
# Description: Client side for managing the 'Falcon_jet game' game
import json
import socket
import threading
import time
...
from constants import SERVER_IP, SERVER_PORT, LOADING_IMG, WHITE_CONTROLS, BLACK_CONTROLS,BACKGROUND_IMG1, LEVEL
import chatlib
...
class Client:
    ...
    def startup_screen(self):
        ...
        # submit_button.pack()
        bt = tk.Button(root, width=27, pady=7, text='Lets GO!!!', bg='#7b7b7b', fg='white', border=0,
        ...
        if cmd != chatlib.PROTOCOL_SERVER['game_status_response']:
            if cmd == chatlib.PROTOCOL_SERVER['winner_msg']:  # Checking if the game ended and there is a winner
                if self.id == int(data):
                    ...
                    #server.winner = None
                    print("STATUSSTATUSSTATUSSTATUS")
                    print(self.id)
                    print(data)
                else:
                ...
                if constants.LEVEL == 3:
                    ...
                    constants.LEVEL+=1
                    #print("THE CONSTANTS LEVEL THE CONSTANTS LEVEL THE CONSTANTS LEVEL " + str(constants.LEVEL))
                    client.start()
            return None
        try:
