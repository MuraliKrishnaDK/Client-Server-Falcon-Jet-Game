# File: server.py
# Description: Server side for managing the 'Falcon_jet game' game
import json
import socket
import threading
import time
...
class Server:
    ...
    def handle_status_message(self, client_socket: socket.socket):
        ...
        ''''if constants.LEVEL>1 and constants.ROUND==1 :
            self.WINNER=None
            constants.ROUND=0'''
        if (self.winner == 0 or self.winner == 1) :#and constants.ROUND == 1 :  # If there is a winner sending a winner message
            #constants.ROUND=0
            #print("Asfsgsagsgsgddhdfhthdh")
    ...
    def start(self) -> None:
        ...
        while True:
            ...
            if self.game.score_0 == 1:
                self.winner = 0
            elif self.game.score_1 == 1:
                self.winner = 1
